...
class VideoDetailActivity : AppCompatActivity() {
    ...
    var mHandler: Handler = object : android.os.Handler() {
    ...
    private fun addMission(playUrl: String?, count: Int) {
        RxDownload.getInstance(this).serviceDownload(playUrl,
                "download$count").subscribe({
            showToast("开始下载")
    ...
    private fun prepareVideo() {
        ...
        gsy_player.setLockClickListener { view, lock -> orientationUtils.isEnable = !lock }
    ...
    private class ImageViewAsynvTask(handler: Handler, activity: VideoDetailActivity, private val mImageView: ImageView) :
        ...
        override fun doInBackground(vararg params: String?): String? {
            var future = Glide.with(mActivity)
                    .load(params[0])
                    .downloadOnly(100, 100)
            try {
                var cacheFile = future.get()
        ...
        override fun onPostExecute(result: String?) {
            ...
            var message = hanlder.obtainMessage()
